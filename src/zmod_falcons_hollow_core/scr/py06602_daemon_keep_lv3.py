...
KEEP_LV3 = 6602
KEEP_LV3_DAEMON_ID = "G_C5D03678_7449_418A_8C49_8D81AF4579DB"
...
class CtrlKeepLv3(ctrl_daemon2.CtrlDaemon2):
	...
	def place_passages(self):
		...
		if (passage):
			...
			print("passage_down = {}".format(passage.id))

		# storage door
		loc, ox, oy = utils_obj.sec2loc(471,484), -11.3137083, 2.82842779
	...
	def place_boss(self):
		...
		if (1):
			wpl = list()
			wpl.append(utils_npc.Waypoint(470, 476, const_toee.rotation_1000_oclock, 1000, utils_npc.WaypointFlag.Delay))
			wpl.append(utils_npc.Waypoint(470, 469, const_toee.rotation_0600_oclock, 1000, utils_npc.WaypointFlag.Delay))
	...
	def place_elite_guards(self):
		...
		npc, ctrl = self.create_npc_at(utils_obj.sec2loc(479, 469), py06603_keep_encounters.CtrlGuardElite, const_toee.rotation_0700_oclock, "keep", "elite1", factions_zmod.FACTION_FIRENDY_NPC, 0, 1)
		utils_npc.npc_description_set_new(npc, None, name_id)

		npc, ctrl = self.create_npc_at(utils_obj.sec2loc(479, 474), py06603_keep_encounters.CtrlGuardElite, const_toee.rotation_0700_oclock, "keep", "elite2", factions_zmod.FACTION_FIRENDY_NPC, 0, 1)
		utils_npc.npc_description_set_new(npc, None, name_id)

		npc, ctrl = self.create_npc_at(utils_obj.sec2loc(492, 480), py06603_keep_encounters.CtrlGuardElite, const_toee.rotation_0200_oclock, "keep", "elite3", factions_zmod.FACTION_FIRENDY_NPC, 0, 1)
		utils_npc.npc_description_set_new(npc, None, name_id)

		if (1):
			wpl = list()
			wpl.append(utils_npc.Waypoint(492, 480, const_toee.rotation_0200_oclock, 1000, utils_npc.WaypointFlag.Delay))
			wpl.append(utils_npc.Waypoint(466, 480, const_toee.rotation_0700_oclock, 1000, utils_npc.WaypointFlag.Delay))
			npc.npc_waypoints_set(wpl, 1)
			npc.npc_flag_set(toee.ONF_WAYPOINTS_DAY)
			npc.npc_flag_set(toee.ONF_WAYPOINTS_NIGHT)
		return
